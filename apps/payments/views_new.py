# For beginners: This file (apps/payments/views_new.py) contains part of the application logic.
...
from apps.payments.serializers_new import (
    PaymentInitiateSerializer, PaymentListSerializer,
    PaymentDetailSerializer, PaymentCallbackSerializer
)
...
def mpesa_callback(request):
    ...
    Called by Daraja after STK push confirmation.
    """
    try:
        ...
        try:
            ...
            if result_code == 0:
                # Success
                payment.status = 'confirmed'
                payment.mpesa_ref = merchant_request_id
                payment.confirmed_at = timezone.now()
                ...
                for item in items:
                    if item.get('Name') == 'MpesaReceiptNumber':
                        payment.mpesa_ref = item.get('Value')
