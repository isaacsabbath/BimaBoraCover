...
def payment_page(request):
    ...
    if request.method == 'POST':
        ...
        payment = Payment.objects.create(
            ...
            amount=amount, payment_type='premium',
            payment_direction='inbound', status='pending',
        )
        try:
            ...
            data = resp.json()
            if data.get('transaction_request_id'):
    ...
    return render(request, 'payments/dashboard.html', {
        'plans': plans, 'active_policy': active_policy, 'payments': payments,
    })
